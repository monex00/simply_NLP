...
def count_genus(genus_list):
    """
    Conta quante volte appare ciascun genus in una lista.
    
    Args:
        genus_list (list): Lista di genus.
        
    Returns:
        dict: Dizionario con i genus come chiavi e il numero di occorrenze come valori.
    """
...
def find_genus(definitions, nlp):
    """
    Estrae il genus dalle definizioni usando il primo nome significativo.
    
    Args:
        definitions (list): Lista di definizioni (stringhe di testo).
        
    Returns:
        list: Lista di genus estratti.
    """
    genus_list = []
    ...
    for definition in definitions:
        ...
        # Analizza la definizione con spaCy
        doc = nlp(definition)
        
        # Cerca il primo sostantivo (genus)
        for token in doc:
            if token.pos_ == "NOUN":  # Cerca il primo nome
                genus_list.append(token.text)
                
        
        # Se non viene trovato un nome, aggiungi un valore nullo
        
    return genus_list

def get_max_genus(genus_count, n):
    """
    Restituisce i n genus con il numero di occorrenze più alto.
    
    Args:
        genus_count (dict): Dizionario con i genus come chiavi e il numero di occorrenze come valori.
        n (int): Numero di genus da restituire.
        
    Returns:
        list: Lista di tuple (genus, conteggio) dei n genus più comuni.
    """
...
#find the 5 best hyponyms of synset based on wup_similarity
...
def overlap_similarity(synset, dataset_definitions):
    # conut overlap of words with count and return a 0 to 1 value
    synset_definitions = synset.examples()
    synset_definitions.append(synset.definition())
...
def get_genus(definition_list, n , term, nlp):
    ...
    # create a dictionary with genus and count
...
#    # Estrai il genus dalle definizioni che sono una colonna di dataset pandas
