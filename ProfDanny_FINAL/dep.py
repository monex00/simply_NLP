...
def execute_subcommand1(records):
    application="java"
    # print(text)
    stringifyText = json.dumps(HistoryEncoder().encode(records))
    result = subprocess.run([application,"-cp", "./simpleNLG-IT/simplenlg-it.jar:./simpleNLG-IT/json-20240303.jar", "./simpleNLG-IT/Record.java", stringifyText], stdout=subprocess.PIPE)
    output = result.stdout.decode('utf-8')
    return output
...
class Record:
    ...
    def __str__(self):
        ...
        missing_dep_str = ""
        for i in range(len(self.missing_dep)):
            missing_dep_str += self.words[i] + " - " + str(self.missing_dep[i]) + "\n"
    ...
    for row in data:
        ...
        if head_index == 0:
            ...
            if head_index not in token_dict:
                head = Token(data[head_index - 1][0], data[head_index - 1][1], data[head_index - 1][2].lower(), data[head_index - 1][3], data[head_index - 1][4], data[head_index - 1][5], data[head_index - 1][6], data[head_index - 1][7], data[head_index - 1][8], data[head_index - 1][9])
        ...
        if int(row[0]) in token_dict:
            ...
            token = Token(row[0], row[1], row[2].lower(), row[3], row[4], row[5], head, row[7], row[8], row[9])
    ...
    for token in tokens:
        if single_slot is not None:
            ...
            for dep in single_dep:
                set_score_dep([dep], token, removed_intention, record, record_index, True)
        else:
...
def main():
    ...
    for i in range(0, number_of_questions):
        ...
        while (frames[i].check_frame() == False):
            ...
            answer_dependencies = get_dependencies(text)
            fill_frame(frames[i], answer_dependencies, removed_intention, record, j, slot,  dep_slot.dependencies if dep_slot is not None else None)
    ...
    final_report = execute_subcommand1(history)
    print(final_report)
